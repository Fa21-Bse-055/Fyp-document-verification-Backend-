...
router.post("/register", upload.single("CNICImage"), async (req, res) => {
  ...
  try {
    ...
    if (
      !normalizedWebUrl.startsWith("http://") &&
      !normalizedWebUrl.startsWith("https://")
    ) {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Error sending email, but continuing with registration:",
          emailSendError
        );
      ...
      bcrypt.genSalt(10, (err, salt) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ msg: "Server Error during password hashing!" });
        }
...
router.post("/addCert", async (req, res) => {
  try {
    const { cert_id, cert_hash } = req.body;
    await certificateModel.create({
      cert_id,
      cert_hash,
    });
    return res.status(200).json({ msg: "Certificate Hash Saved Successfully" });
  } catch (e) {
...
router.get("/getCert", async (req, res) => {
  try {
    ...
    return res.status(200).json({ data: foundCertificate });
  } catch (e) {
    return res.status(400).json({ msg: e.message });
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ msg: "Server error during document upload" });
    }
  }
);
...
router.get("/my-documents", isOrganization, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ msg: "Server error while fetching documents" });
  }
});
...
router.get("/document/:id", isOrganization, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ msg: "Server error while fetching document" });
  }
});
